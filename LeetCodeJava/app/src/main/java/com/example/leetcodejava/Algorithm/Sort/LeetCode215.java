...
/**********************************************方法四*****************************************************************/
     ...
     * 上文中提到的 「划分」 过程是：从子数组 a[l⋯r] 中选择任意一个元素 x 作为主元，调整子数组的元素使得左边的元素都小于等于它，右边的元素都大于等于它， x的最终位置就是 q。
     ...
     * 就直接返回 a[q]；否则，如果 q比目标下标小，就递归右子区间，否则递归左子区间。
    ...
    /**
     *  为了应对极端测试用例，例如:8 7 6 5 4 3 2 1完全逆序数列
     *  使得递归树加深，可以在循环一开始的时候，随机选择一个元素作为基准元素，并且让基准元素和数列首位元素交换位置(此处交换的最后一个元素)
     */
    ...
    /**
     * 将数组的某一段元素进行划分，小的在左边，大的在右边(双边循环)
     */
